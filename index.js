...
function displayCheat(type,title,cheatObj){
	...
	if(cheatObj === undefined){
		switch(type){
			case 'html':
				...
				break;
			case 'css':
				console.log("[\033[33;1mNOT FOUND\033[0m] The CSS property that you requested doesn't exist/isn't indexed");
				break;
			case 'js':
	...
	titleBar += "┓\n┃\u001b[33;1m "
	titleBar += wrap(title,{width:60}).padEnd(71); 
	titleBar += "\u001b[0m┃\n"
	titleBar += "┣" + lineSeparator + "┫"
	...
	for(let element in cheatObj) {
		...
		console.log("┃ ● \033[4;33m" + element + "\033[0m" + "┃".padStart(70 - element.length));
		// console.log("┃");
		body = wrap(body,{width:65}).split("\n");
		console.log("┃" + " ".repeat("72") + "┃");
		for(let e of body){
...
function getCheat(getArgs){
	...
	fs.readFile(`cheats/en/${command.type}.json`,(err,data) => {
		if(err) {} 
			...
			displayCheat(command.type,command.value,bigList[command.value]);
